...
std::vector<double> QPSKdemod_LLR_Rayleigh(const std::vector<std::complex<double>> &input_signal, const std::vector<double> &fading_mu, double SNR, double R) {
    ...
    for (size_t i = 0; i < length; i++) {
        double d00 = std::norm(input_signal[i] - fading_mu[i] * std::complex<double>(1.0 * norm2, 1.0 * norm2));
        double d10 = std::norm(input_signal[i] - fading_mu[i] * std::complex<double>(-1.0 * norm2, 1.0 * norm2));
        double d01 = std::norm(input_signal[i] - fading_mu[i] * std::complex<double>(1.0 * norm2, -1.0 * norm2));
        double d11 = std::norm(input_signal[i] - fading_mu[i] * std::complex<double>(-1.0 * norm2, -1.0 * norm2));

        output_bits[2 * i] = calculateLLR_Rayleigh(SNR, std::min(d00, d01) - std::min(d10, d11), R, 4);
        output_bits[2 * i + 1] = calculateLLR_Rayleigh(SNR, std::min(d00, d10) - std::min(d01, d11), R, 4);
    }
