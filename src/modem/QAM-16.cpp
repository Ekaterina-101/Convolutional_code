...
std::vector<double> QAM16demod_LLR_Rayleigh(const std::vector<std::complex<double>> &input_signal, const std::vector<double> &fading_mu, double SNR, double R) {
    ...
    for (size_t i = 0; i < length; i++) {
        ...
        distances[0] = std::abs(input_signal[i] - fading_mu[i] * std::complex<double>(1.0 * norm4, 1.0 * norm4));    // 0000
        distances[1] = std::abs(input_signal[i] - fading_mu[i] * std::complex<double>(1.0 * norm4, 3.0 * norm4));    // 0001
        distances[2] = std::abs(input_signal[i] - fading_mu[i] * std::complex<double>(1.0 * norm4, -1.0 * norm4));   // 0010
        distances[3] = std::abs(input_signal[i] - fading_mu[i] * std::complex<double>(1.0 * norm4, -3.0 * norm4));   // 0011
        distances[4] = std::abs(input_signal[i] - fading_mu[i] * std::complex<double>(3.0 * norm4, 1.0 * norm4));    // 0100
        distances[5] = std::abs(input_signal[i] - fading_mu[i] * std::complex<double>(3.0 * norm4, 3.0 * norm4));    // 0101
        distances[6] = std::abs(input_signal[i] - fading_mu[i] * std::complex<double>(3.0 * norm4, -1.0 * norm4));   // 0110
        distances[7] = std::abs(input_signal[i] - fading_mu[i] * std::complex<double>(3.0 * norm4, -3.0 * norm4));   // 0111
        distances[8] = std::abs(input_signal[i] - fading_mu[i] * std::complex<double>(-1.0 * norm4, 1.0 * norm4));   // 1000
        distances[9] = std::abs(input_signal[i] - fading_mu[i] * std::complex<double>(-1.0 * norm4, 3.0 * norm4));   // 1001
        distances[10] = std::abs(input_signal[i] - fading_mu[i] * std::complex<double>(-1.0 * norm4, -1.0 * norm4)); // 1010
        distances[11] = std::abs(input_signal[i] - fading_mu[i] * std::complex<double>(-1.0 * norm4, -3.0 * norm4)); // 1011
        distances[12] = std::abs(input_signal[i] - fading_mu[i] * std::complex<double>(-3.0 * norm4, 1.0 * norm4));  // 1100
        distances[13] = std::abs(input_signal[i] - fading_mu[i] * std::complex<double>(-3.0 * norm4, 3.0 * norm4));  // 1101
        distances[14] = std::abs(input_signal[i] - fading_mu[i] * std::complex<double>(-3.0 * norm4, -1.0 * norm4)); // 1110
        distances[15] = std::abs(input_signal[i] - fading_mu[i] * std::complex<double>(-3.0 * norm4, -3.0 * norm4)); // 1111

        double bit0 = std::min({distances[0], distances[1], distances[2], distances[3], distances[4], distances[5], distances[6], distances[7]});
        double bit1 = std::min({distances[8], distances[9], distances[10], distances[11], distances[12], distances[13], distances[14], distances[15]});
        output_bits[4 * i] = calculateLLR_Rayleigh(SNR, bit0 - bit1, R, 16);

        bit0 = std::min({distances[0], distances[1], distances[2], distances[3], distances[8], distances[9], distances[10], distances[11]});
        bit1 = std::min({distances[4], distances[5], distances[6], distances[7], distances[12], distances[13], distances[14], distances[15]});
        output_bits[4 * i + 1] = calculateLLR_Rayleigh(SNR, bit0 - bit1, R, 16);

        bit0 = std::min({distances[0], distances[1], distances[4], distances[5], distances[8], distances[9], distances[12], distances[13]});
        bit1 = std::min({distances[2], distances[3], distances[6], distances[7], distances[10], distances[11], distances[14], distances[15]});
        output_bits[4 * i + 2] = calculateLLR_Rayleigh(SNR, bit0 - bit1, R, 16);

        bit0 = std::min({distances[0], distances[2], distances[4], distances[6], distances[8], distances[10], distances[12], distances[14]});
        bit1 = std::min({distances[1], distances[3], distances[5], distances[7], distances[9], distances[11], distances[13], distances[15]});
        output_bits[4 * i + 3] = calculateLLR_Rayleigh(SNR, bit0 - bit1, R, 16);
    }
