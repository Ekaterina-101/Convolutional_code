...
std::vector<std::vector<int>> soft_serial_lva(const Trellis &T, std::vector<double> &encoded, int L) {
    ...
    auto find_second_best_to_path = [&](const PathInfo &path, int max_time, int &out_divergence_time, int &out_prev_state, int &out_input, double &out_cost) -> bool {
    ...
    auto cmp = [](const Candidate &a, const Candidate &b) { return std::get<0>(a) > std::get<0>(b); };
...
std::vector<int> soft_serial_lva_final(const Trellis &T, std::vector<double> &encoded, int L, int CRC, int size_block_CRC) {
    auto res = soft_serial_lva(T, encoded, L);
