...
std::vector<std::vector<int>> soft_serial_lva(const Trellis &T, std::vector<double>& encoded, int L) {
    ...
    // ====================================================================
    // ШАГ 1: Прямой проход Viterbi (находим 1-й путь)
    // ====================================================================
    ...
    for (int i = 0; i < (int)length; i++) {
        ...
        for (int curr_state = 0; curr_state < N; curr_state++) {
            if (phi_best[curr_state][i] == std::numeric_limits<double>::infinity()) continue;
    ...
    // ====================================================================
    // ШАГ 3: Функция для вычисления второго лучшего пути к заданному
    // ====================================================================
    
    // Для заданного пути и отрезка [0, merge_time] находим второй лучший путь
    // Второй лучший путь = путь, который отличается от заданного хотя бы в одном переходе
    auto find_second_best_to_path = [&](const PathInfo& path, int max_time, 
        ...
        // Ищем момент, где можно отклониться
        ...
        for (int t = 1; t <= max_time; t++) {
            ...
            // Ищем альтернативный переход в curr_state
            for (int l = 0; l < N; l++) {
                if (l == prev_state_in_path) continue;
                if (phi_best[l][t - 1] == std::numeric_limits<double>::infinity()) continue;
                
                for (int input = 0; input < 2; input++) {
                    if (input == input_in_path && l == prev_state_in_path) continue;
                    if (T.nextStates[l][input] != curr_state) continue;
                    ...
                    // Полная стоимость = метрика до t-1 + метрика ветви + стоимость после t
                    double remaining_cost = 0.0;
                    if (t <= (int)length) {
                        // Стоимость от t до конца берём из path (остаток пути совпадает с исходным)
                        // Вычисляем её
    ...
    // ====================================================================
    // ШАГ 4: Куча кандидатов (min-heap по стоимости)
    // ====================================================================
    
    using Candidate = std::tuple<double, int, int, int, int>; // cost, path_index, merge_time, prev_state, input
    auto cmp = [](const Candidate& a, const Candidate& b) {
        return std::get<0>(a) > std::get<0>(b);
    };
    std::priority_queue<Candidate, std::vector<Candidate>, decltype(cmp)> pq(cmp);
    
    // Инициализация: все прямые отклонения от 1-го пути
    for (int j = 1; j <= (int)length; j++) {
        int best_prev = best_states[j - 1];
        int best_curr = best_states[j];
        
        for (int l = 0; l < N; l++) {
            if (l == best_prev) continue;
            if (phi_best[l][j - 1] == std::numeric_limits<double>::infinity()) continue;
            
            for (int input = 0; input < 2; input++) {
                if (T.nextStates[l][input] != best_curr) continue;
    ...
    // ====================================================================
    // ШАГ 5: Последовательное извлечение путей
    // ====================================================================
    
    std::vector<std::vector<int>> result(L);
    result[0] = best_inputs;
    
    std::set<std::vector<int>> unique_paths;  // для предотвращения дубликатов
    ...
    for (int k = 1; k < L && !pq.empty(); k++) {
        ...
        // Восстанавливаем путь из parent_idx + отклонение в merge_t
        const PathInfo& parent = found_paths[parent_idx];
        
        std::vector<int> path(length);
        
        // Часть после слияния (от merge_t до конца) — как у родителя
        for (int i = merge_t; i < (int)length; i++) {
            path[i] = parent.inputs[i];
        }
        
        // Переход в точке слияния
        path[merge_t - 1] = pred_input;
        
        // Часть до слияния — наилучший путь до pred_state
        {
        ...
        // Вычисляем последовательность состояний для этого пути
        ...
        // Находим момент последнего слияния с родителем
        ...
        // Сохраняем информацию о пути для будущих итераций
        ...
        // ================================================================
        // ДОПОЛНЕНИЕ: генерируем "второй лучший путь" к этому пути
        // ================================================================
        
        // Находим второй лучший путь к новому пути на участке [0, final_merge]
        ...
        if (find_second_best_to_path(new_path, final_merge, div_time, alt_prev_state, alt_input, alt_cost)) {
            // Добавляем кандидата в кучу
            double total_with_cost = alt_cost;  // find_second_best_to_path уже посчитал полную стоимость
            pq.push({total_with_cost, new_idx, div_time, alt_prev_state, alt_input});
        }
        
        // Также добавляем прямые отклонения от этого пути (как в вашей версии для 1-го пути)
        for (int j = 1; j <= final_merge; j++) {
            int curr_state_in_path = new_path.states[j];
            
            for (int l = 0; l < N; l++) {
                if (l == new_path.states[j - 1]) continue;
                if (phi_best[l][j - 1] == std::numeric_limits<double>::infinity()) continue;
                
                for (int input = 0; input < 2; input++) {
                    if (T.nextStates[l][input] != curr_state_in_path) continue;
                    if (input == new_path.inputs[j - 1] && l == new_path.states[j - 1]) continue;
                    ...
                    // Вычисляем оставшуюся стоимость от j до конца
    ...
    // Заполняем оставшиеся пути (если не нашли L)
