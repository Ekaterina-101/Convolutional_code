...
std::vector<std::complex<double>> QAM64mod(const std::vector<int>& input_bits) {
    ...
    for (size_t i = 0; i < length; i++) {
        ...
        double real_part = (1 - 2 * b0) * (1 + 2 * b1 + 4 * b2) * norm; //(7 - (b0 << 3) - (b1 << 2) - (b2 << 1)) * norm;
        double imag_part = (1 - 2 * b3) * (1 + 2 * b4 + 4 * b5) * norm;
                    ...
                    for (int b3 = 0; b3 <= 1; b3++) {
                        for (int b4 = 0; b4 <= 1; b4++) {
                            for (int b5 = 0; b5 <= 1; b5++) {
                                double real_part = (1 - 2 * b0) * (2 * b1 + 4 * b2 + 1) * norm;
                                double imag_part = (1 - 2 * b3) * (2 * b4 + 4 * b5 + 1) * norm;
